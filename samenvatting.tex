...
\begin{document}
	...
	\subsection{\indx{Stijve differentiaalvergelijkingen}}
		...
		\begin{define}[\indx{Superstabiel}]
	...
	\newpage
	\subsection{Numerieke methoden voor \indx{begin-randwaardeproblemen}}
		We beschouwen een \indx{instationaire vergelijking}. Deze heeft een integraalvorm en differentiaalvorm, waarbij de laatste hogere eisen aan differentieerbaarheid stelt.

		Deze vergelijking is van de vorm ($y=y(x,t)$)
		\[
			\frac{\partial y}{\partial t} = \frac{\partial^2 y}{\partial x^2} \,,
		 \]
		 met de voor de linkerkant vereiste beginvoorwaarde $y(x,t=0)=y_0(x)$ en de voor de rechterkant vereiste randvoorwaarden $y(x=0,t)=y_l (t) ,\ y(x=1,t)=y_r(t)$.
		 Merk op dat dit \textbf{volledig continu} is.

		 Nu gaat we in twee stappen discretiseren,
		 \begin{itemize}
		 	\item in de $x$-richting (hoofdstuk 7)
		 	\item in de tijd (hoofdstuk 6).
		 \end{itemize}

		 In de eerste stap maken we een \textbf{semi-discrete} vergelijking
		 \[
			 \frac{\d u_i}{\d t} = \frac{u_{i-1}-2u_i+u_{i+1}}{h^2} \quad u_i=u_i(t).
		  \]
		  Nu zijn de randwaarden $u_{i=0}(t)=y_l(t)$ en $u_{i=n+1}=y_r(t)$, en de beginwaarde $u_i(t=0) = y_0 (t)$.

		  Dit is te schrijven in vectorvorm als
		  \[
			  \frac{\d \u }{\d t} = K \u + \r
		   \]
		   waarbij
		   \[
			   ...
			   \vdots \\
				0 \\
				y_r (t) /h^2}, \ \u = \vvec{u_1(t) \\ u_2(t) \\ \vdots \\ u_n(t)}.
		    \]
		    Om nu een \textbf{volledig discrete} vergelijking te krijgen, gebruiken we bijvoorbeeld Voorwaarts Euler, zodat
		    \[
			    \frac{w_i^{j+1}-w_i^j}{\Delta t} = \frac{w_{i-1}^j -2w_i^j +w_{i+1}^j}{h^2} + r_i^j \,.
		     \]
		     Hieruit zien we het \indx{differentiemolecuul} of \indx{stencil} van Voorwaarts Euler, waarin we het roosterpunt $w_i^{j+1}$ uitrekenen aan de hand van drie roosterpunten daaronder: $w_{i-1}^j,\ w_i^j$ en $w_{i+1}^j$.

		     \subsubsection{Stabiliteitseis $\Delta t$}
			     We hebben $ \frac{\d \y }{\d t} = A\y + \g (t) $. Er volgt $ \Delta t \le \frac{2}{\lambda_{\max}} $, waarbij $\lambda_{\max}$ de grootste eigenwaarde is van $A$. We schatten $\lambda_{\max}$ met Gershgorin, hieruit volgt $ \lambda_{\max} \le \frac{4}{h^2} $. Dit geeft $ \Delta t \le \frac{2}{\frac{4}{h^2}} = \frac{h^2}{2} $. We zien dat er voor Voorwaarts Euler strenge stabiliteitseisen gelden ($ h \to \frac{h}{2} \implies \Delta t \to \frac{\Delta t}{4} $), oftewel VE is duur. Maar wel eenvoudig.

			     \textbf{Achterwaarts Euler.} Als we voor Achterwaarts Euler hetzelfde doen, zien we dat AE voor $ \Delta t $ onvoorwaardelijk stabiel is.

		    %todo not sure what the rest is about?

	\section{College 10}
		\subsection{Bepaling nulpunten niet-lineaire vergelijkingen}
			Zij $f(x)$ een functie. Stel $ f(p) = 0 $, dan noemen we $p$ een \indx{nulpunt} van de functie. Om nulpunten te bepalen/benaderen kunnen we de volgende methodes gebruiken.
			\begin{itemize}
				\item Bisectiemethode
				\item Vast punt
			...
			\subsubsection{Bisectiemethode \textmd{(Gebaseerd op de tussenwaardenstelling)}}
				...
				\[
					...
					\end{cases}
				\]
				Dit zetten we door, en hiermee sluiten we een nulpunt $p$ in met een vooraf bepaalde nauwkeurigheid.
			...
			\subsubsection{Vast punt (fixed point)}
				Een punt $p$ heet een \indx{vast punt} als geldt
				\[
					g(p)=p\,.
				\]
				Er is altijd een functie $g(x)$ te vinden zodanig dat een vast punt van $g$ een nulpunt van $f$ is, bijvoorbeeld $g(x)=x-cf(x),\ c\ne 0$.
				\begin{theonum}\label{10-1}
					Zij $g(x)$ continu op $ [a,b] $, en $ g(x) \in [a,b], x \in [a,b] $. Dan is er tenminste \'e\'en vast punt $ p \in [a,b] $.
				\end{theonum}

				\begin{theonum}[Gevolg van stelling \ref{10-1}]\label{10-2}
					Als $ \abs{g'(x)} < 1 $, dan is $p$ een uniek vast punt.
				\end{theonum}
				We kunnen punten ook beschrijven door middel van \indx{iteraties}. Als een punt een vast punt is geldt er
				\[
			...
			\subsubsection{Newton-Raphson iteratie}
				...
				\]
				Neem aan dat $ (p-p_n) \ll 1$. Dan geldt $ f(p_n) + (p-p_n)f'(p_n) \approx 0 \implies p_{n+1} \approx p_n - \frac{f(p_n)}{f'(p_n)} $. Dit noemen we \indx{Newton-Raphson}, en we noemen $ \frac{f(p_n)}{f'(p_n)} $ het \indx{residu}.

				Kies een $ p_0 $ en bereken $ f(p_0), f'(p_0) \to p_1$ etc.

				\begin{opm}
					Vastepunt-iteraties garanderen \textbf{geen} convergentie. Wel zijn ze uitbreidbaar naar stelsels.
				\end{opm}
        \newpage
	\section{College 11}
